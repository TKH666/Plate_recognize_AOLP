...
from model.model_net import ONet
import time
import copy
...
train_path = './data/trainOnet/anno_store/imglist_anno_onet.txt'
val_path = './data/valOnet/anno_store/imglist_anno_onet_val.txt'
batch_size = 64
...
train_logging_file = './train_Net/Onet_train_logging.txt'
...
for epoch in range(num_epochs):
    ...
    for phase in ['train', 'val']:
        ...
        print('{} Loss: {:.4f} 车牌分类accuracy: {:.4f} 车牌分类Loss Loss: {:.4f} 回归问题 offset Loss: {:.4f}'
              .format(phase, epoch_loss, epoch_accuracy, epoch_loss_cls, epoch_loss_offset))
        with open(train_logging_file, 'a') as f:
            f.write('{} Loss: {:.4f}  车牌分类accuracy: {:.4f} 车牌分类 Loss: {:.4f} 回归问题 offset Loss: {:.4f}'
                    .format(phase, epoch_loss, epoch_accuracy, epoch_loss_cls, epoch_loss_offset)+'\n')
        f.close()
...
torch.save(model.state_dict(), './model/onet_Weights')